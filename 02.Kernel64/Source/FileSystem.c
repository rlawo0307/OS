...
int kFindDirectoryEntry( const char* pcFileName, DIRECTORYENTRY* pstEntry )
{
    ...
    for( i = 0 ; i < FILESYSTEM_MAXDIRECTORYENTRYCOUNT ; i++ )
    {
        temp = kStrLen(pstRootEntry[i].vcFileName);
        if(iLength == temp && kMemCmp( pstRootEntry[ i ].vcFileName, pcFileName, iLength ) == 0)
        {
        ...
        int* piDirectoryEntryIndex )
{
    ...
    *piDirectoryEntryIndex = kFindFreeDirectoryEntry();
    ...
    pstEntry->group = 6;
    pstEntry->other = 4;
    kReadRTCTime( &(pstEntry -> bHour), &(pstEntry -> bMinute), &(pstEntry -> bSecond) );
    kReadRTCDate( &(pstEntry -> wYear), &(pstEntry -> bMonth), &(pstEntry -> bDayOfMonth), &(pstEntry -> bDayOfWeek) );
...
FUSER* kOpenUserFile( const char* pcFileName, const char* pcMode ,int count,char* id, char* pwd, int idlen, int pwdlen )
{
   ...
   for (int i = 0; i<idlen;i++){
    ...
    pstFile->Passwd[i] = pwd[i];
    
    }
    pstFile->Passwd[pwdlen] = '\0';

    pstFile->usernum = count;
    pstFile->Idlen = idlen;
    pstFile->Pwdlen = pwdlen;
...
{
    // 전체 영역을 초기화
    kMemSet( pstFile, 0, sizeof( FUSER) );
...
void kUpdateFileMod(DIRECTORYENTRY pstEntry,int u_mod, int g_mod, int o_mod,int index){
   ...
   // pstEntry->user = &u_mod;
    pstEntry.user = u_mod;
    pstEntry.group = g_mod;
    pstEntry.other = o_mod;
    kSetDirectoryEntryData( index, &pstEntry );
...
DIR* kOpenDirectory( const char* pcDirectoryName )
{
    ...
    if(kMemCmp(pcDirectoryName,"/",2)!= 0){
        if( iDirectoryEntryOffset == -1)
        {
 ...
 */
void kSetDotInDirectory(dwCluster){
    DIRECTORYENTRY stEntry;
    int iDirectoryEntryOffset = 0;
    // kPrintf("%d %d", currentClusterIndex, dwCluster);

    kMemCpy( stEntry.vcFileName, ".", 2 );
    stEntry.dwStartClusterIndex = lastCluster;
    // kPrintf("chekc11111: %d", lastCluster);
    stEntry.dwFileSize = 0;
    stEntry.flag=1;
    if(currentClusterIndex == 0){
        ...
        stEntry.ParentDirectoryCluserIndex = dwCluster;
    }
    ...
    stEntry.group = 7;
    stEntry.other = 5;
    kReadRTCTime( &(stEntry.bHour), &(stEntry.bMinute), &(stEntry.bSecond) );
    kReadRTCDate( &(stEntry.wYear), &(stEntry.bMonth), &(stEntry.bDayOfMonth), &(stEntry.bDayOfWeek) );
       stEntry.user = 7;
    stEntry.group = 7;
    stEntry.other = 5;
    kReadRTCTime( &(stEntry.bHour), &(stEntry.bMinute), &(stEntry.bSecond) );
    ...
    stEntry.dwStartClusterIndex = lastCluster;
    // kPrintf("chekc2222: %d", lastCluster);
    stEntry.dwFileSize = 0;
    stEntry.flag=1;
    //kchTodo
    stEntry.ParentDirectoryCluserIndex = currentClusterIndex;
    stEntry.ParentDirectoryPath[0] = '/';
    stEntry.ParentDirectoryPath[1] = '\0';
