//TODO kch : testk is for test plz remove
#include "Task.h"
...
static void kInitializeTCBPool( void )
{
    ...
    kMemSet( (TCB*)TASK_TCBPOOLADDRESS, 0, sizeof( TCB ) * TASK_MAXCOUNT );
                  ...
                  QWORD qwEntryPointAddress )
{
    ...
    if( qwFlags & TASK_FLAGS_THREAD )
    {
        ...
        // pstTask->ticket = kCalculateTicket(qwFlags);
    ...
    //TODO : kch 위치 여기 아니면 kAddTask
    pstTask->pass = kGetMinPass(gs_stScheduler.vstReadyList);
...
static TCB* kGetNextTaskToRun( void )
{
    ...
    QWORD totaltickets = 0;
    
    for( i = 0; i < TASK_MAXREADYLISTCOUNT; i++){
        iTaskCount = kGetListCount( &( gs_stScheduler.vstReadyList[ i ] ) );
        totaltickets += (iTaskCount * kCalculateTicket(i));

    }

    testk++;
    //RR Scheduler
    pstTarget = kRRScheduler();
    //Lottery Scheduler
    // pstTarget = kLotterySchedular(totaltickets);
    //Stride Scheduler
    // pstTarget = kStrideSchedular();
    return pstTarget;
}

static TCB* kRRScheduler( void )
{
    TCB* pstTarget = NULL;
    int iTaskCount, i, j;
...
static TCB* kLotterySchedular( int totaltickets )
{
    TCB* pstTarget = NULL;
    TCB* current = NULL;
    int iTaskCount, i, j;
    QWORD winner = 0, counter = 0;

    winner = kGetRandom(totaltickets);

    for(i = 0; i < TASK_MAXREADYLISTCOUNT ; i++){
        if ( kGetListCount( &( gs_stScheduler.vstReadyList[ i ])) > 0){
            current = (TCB*)kGetHeaderFromList(&( gs_stScheduler.vstReadyList[ i ]));
             while(current){
                counter += (current->ticket);
                if(counter >= winner){
                    pstTarget = kRemoveList(&(gs_stScheduler.vstReadyList[i]), current->stLink.qwID);
                    break; 
                }
            current = kGetNextFromList(&( gs_stScheduler.vstReadyList[ i ] ), current);
            }
        }
        // 만약 수행할 태스크를 찾았으면 종료
        if( pstTarget != NULL ){
    ...
    // if(testk >= 20000 && testk % 10000 == 0)
    //     kPrintf("qq: %d\n", winner);
    return pstTarget;
}
static TCB* kStrideSchedular( void )
{
    TCB* pstTarget = NULL;
	TCB* current = NULL;
	QWORD minPass;

	minPass = kGetMinPass(gs_stScheduler.vstReadyList);
    ...
    for(int i=0; i<TASK_MAXREADYLISTCOUNT; i++)
	{
		...
		while(current)
		{
			if(current->pass == minPass)
			{
				pstTarget = kRemoveList(&(gs_stScheduler.vstReadyList[i]), current->stLink.qwID);
				(pstTarget->pass) += pstTarget->stride;
				//kPrintf("%q %d\n", pstTarget->stLink.qwID, pstTarget->pass);
				return pstTarget;
			}
			else
			{
...
static BOOL kAddTaskToReadyList( TCB* pstTask )
{
    ...
    pstTask->ticket = kCalculateTicket(bPriority);
    pstTask->stride = 1000 / pstTask->ticket;
...
BOOL kChangePriority( QWORD qwTaskID, BYTE bPriority )
{
    ...
    if( pstTarget->stLink.qwID == qwTaskID )
    {
        ...
        pstTarget->ticket = kCalculateTicket(bPriority);
        pstTarget->stride = 1000 / pstTarget->ticket;
...
void kSchedule( void )
{
    ...
    //kch
    pstRunningTask->qwSpendProcessorTimeInIdleTask = gs_stScheduler.qwSpendProcessorTimeInIdleTask;
...
BOOL kScheduleInInterrupt( void )
{
    ...
    //kch
    pstRunningTask->qwSpendProcessorTimeInIdleTask += TASK_PROCESSORTIME;
